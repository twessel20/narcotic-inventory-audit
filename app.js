...
function countTable(a){
 return '<div class="audit-card audit-section-card"><div class="audit-section-head"><div><span class="kicker">PHYSICAL COUNT</span><h3>Physical inventory</h3><p class="meta">Enter the physically verified vial count for each storage location.</p></div></div><div class="audit-grid-wrap"><div class="audit-grid"><div class="audit-grid-corner">Medication</div>'+LOCS.map(l=>'<div class="audit-grid-head">'+l+'</div>').join('')+MEDS.map(m=>'<div class="audit-med-label">'+m+'</div>'+LOCS.map(l=>'<div class="audit-count-cell"><input aria-label="'+m+' '+l+'" type="number" min="0" step="1" inputmode="numeric" data-count-loc="'+l+'" data-count-med="'+m+'" value="'+Number(a.counts?.[l]?.[m]||0)+'"></div>').join('')).join('')+'</div></div></div>'
}
...
async function reportHtml(r){
 ...
 '<section class="report-attestation"><h2>Final overall controlled-substance audit attestation</h2><p>'+esc(r.attestationText||'I certify that I have personally reviewed this controlled-substance audit, the physical counts and seal records, and each location’s signer and witness certifications. To the best of my knowledge, this record is complete, accurate, and truthful.').replace(/\n/g,'<br>')+'</p><div class="report-final-signature"><div class="report-signature-label">FINAL CERTIFYING AUDITOR SIGNATURE</div>'+(r.attestationSignature?'<img src="'+r.attestationSignature+'" alt="Final certifying auditor signature">':'')+'<div class="report-signature-name">'+esc(r.attestationName||'')+'</div></div></section>'+
 '<section class="report-notes"><h2>Audit notes</h2><p>'+esc(r.notes||'').replace(/\n/g,'<br>')+'</p><p>'+esc(r.usageSummary||'').replace(/\n/g,'<br>')+'</p></section>'+
