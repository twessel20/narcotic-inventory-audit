...
function countTable(a){
 return '<div class="audit-card audit-section-card"><div class="audit-section-head"><div><span class="kicker">PHYSICAL COUNT</span><h3>Inventory comparison</h3><p class="meta">Last audit values are shown for reference. Enter the physically verified current vial count.</p></div></div><div class="audit-grid-wrap"><div class="audit-grid audit-grid-compare"><div class="audit-grid-corner">Medication</div>'+LOCS.map(l=>'<div class="audit-grid-head">'+l+'<small>Last / Current</small></div>').join('')+MEDS.map(m=>'<div class="audit-med-label">'+m+'</div>'+LOCS.map(l=>{const p=a.priorCounts?.[l]?.[m];return '<div class="audit-count-cell"><span class="prior-count">'+(p==null?'—':Number(p))+'</span><span class="count-sep">/</span><input aria-label="'+m+' '+l+' current count" type="number" min="0" step="1" inputmode="numeric" data-count-loc="'+l+'" data-count-med="'+m+'" value="'+Number(a.counts?.[l]?.[m]||0)+'"></div>'}).join('')).join('')+'</div></div></div>'
}
function breakawayTagTable(a){
 return '<div class="audit-card audit-section-card"><div class="audit-section-head"><div><span class="kicker">SECURITY SEALS</span><h3>Breakaway tag record</h3><p class="meta">Record the tag number you removed and the new tag number you installed at each audited location.</p></div></div><div class="tag-entry-list">'+LOCS.map(l=>{const t=a.breakawayTags?.[l]||{};return '<div class="tag-entry-card"><div class="tag-entry-title">'+l+'</div><div class="tag-entry-fields"><label>Tag found / removed<input inputmode="numeric" autocomplete="off" placeholder="Enter removed tag #" data-tag-loc="'+l+'" data-tag-kind="foundRemoved" aria-label="'+l+' tag found or removed" value="'+esc(t.foundRemoved||'')+'"></label><label>New tag installed<input inputmode="numeric" autocomplete="off" placeholder="Enter new tag #" data-tag-loc="'+l+'" data-tag-kind="newInstalled" aria-label="'+l+' new tag installed" value="'+esc(t.newInstalled||'')+'"></label></div></div>'}).join('')+'</div></div>'
}
