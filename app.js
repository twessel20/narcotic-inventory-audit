...
function breakawayTagTable(a){
 return '<div class="audit-card audit-section-card"><div class="audit-section-head"><div><span class="kicker">SECURITY SEALS</span><h3>Breakaway tag record</h3><p class="meta">Record the tag found/removed and the new tag installed at each audited location.</p></div></div><div class="tag-entry-grid"><div class="tag-entry-head">Location</div><div class="tag-entry-head">Tag found / removed</div><div class="tag-entry-head">New tag installed</div>'+LOCS.map(l=>{const t=a.breakawayTags?.[l]||{};return '<div class="tag-location">'+l+'</div><input inputmode="numeric" autocomplete="off" data-tag-loc="'+l+'" data-tag-kind="foundRemoved" aria-label="'+l+' tag found or removed" value="'+esc(t.foundRemoved||'')+'"><input inputmode="numeric" autocomplete="off" data-tag-loc="'+l+'" data-tag-kind="newInstalled" aria-label="'+l+' new tag installed" value="'+esc(t.newInstalled||'')+'">'}).join('')+'</div></div>'
}
